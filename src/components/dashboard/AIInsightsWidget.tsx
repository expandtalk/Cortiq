...
export function AIInsightsWidget({ siteId }: AIInsightsWidgetProps) {
  ...
  const handleGenerateInsights = async () => {
    try {
      ...
      toast.success(`Genererade ${result.insights?.length || 0} nya insikter!`);
    } catch (err) {
      toast.error('Kunde inte generera insikter. Försök igen senare.');
    }
  };
    ...
    return (
      <Card>
        ...
        <CardContent>
          <Alert>
            <AlertDescription>
              Kunde inte ladda insikter: {error}
            </AlertDescription>
          </Alert>
        </CardContent>
      </Card>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-2">
            ...
            <CardTitle>AI Insikter</CardTitle>
            {highPriorityCount > 0 && (
              <Badge variant="destructive">{highPriorityCount} hög prioritet</Badge>
            )}
          </div>
          <Button 
          ...
          >
            ...
            {generating ? 'Genererar...' : 'Generera nya'}
          </Button>
        </div>
        <CardDescription>
          Automatiskt genererade rekommendationer baserat på din data
        </CardDescription>
      </CardHeader>
      <CardContent>
        {!hasInsights ? (
          ...
          <div className="space-y-4">
              ...
              <div key={insight.id} className="border rounded-lg p-4 relative">
                ...
                <div className="flex justify-between items-center mt-3 pt-2 border-t">
                  <span className="text-xs text-muted-foreground">
                    Säkerhet: {insight.confidence_score}%
                  </span>
                  <span className="text-xs text-muted-foreground">
            ...
            {insights.length > 3 && (
              <p className="text-center text-sm text-muted-foreground">
                +{insights.length - 3} fler insikter tillgängliga
              </p>
            )}
          </div>
        )}
