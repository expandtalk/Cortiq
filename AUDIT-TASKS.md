...
>
> **Läge 2026-07-16 (uppdaterad efter merge + deploy):** `[x]` P0-1, P1-1, P0-3, P1-2, P1-3, P1-5, P1-6, P1-7, P2-2, P2-7, P2-9, P2-10 — **alla mergade till `main` (PR #9) och utrullade** (edge functions + FTP + migration). `[~]` P1-4 (dok live, men DU måste skicka 30-dagarsvarslet innan Cloudflare aktiveras på kundsajter). `[ ]` P0-2 (blockerad på `/mcp`) + P2-1/3/4/5/6/8/11/12/13/14 + alla P3.
>
> **Bonusleverans 2026-07-16:** delete-site-feature (radera sajt från dashboarden, hård radering + domän-bekräftelse) byggd, mergad (PR #9) och deployad (FTP + `event_queue`-migration applicerad via dashboard SQL, `confdeltype='c'`). Spec/plan i `docs/superpowers/`.
>
> **Git-drift stängd:** `main` matchar nu produktion; brancherna `audit-2026-07-15-fixes` och `feat/delete-site` borttagna.
...
- [x] **P0-3 `[SEC][BIZ]` Hela 2026-07-15-auditens arbete var OCOMMITTAT — deployad produktion matchade ingen commit (git-drift).** `(NY, 2026-07-16)` — **✅ LÖST 2026-07-16: committat, pushat och mergat till `main` via PR #9; `main` matchar nu produktion. Redundanta brancher borttagna.**
...
- [x] **P1-2 `[GDPR]` "Cookieless / consent-exempt"-läget spelar in klick/scroll/heatmaps + konvertering utan samtycke — överskrider audience-measurement-undantaget.** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16 (val #1 aggregerad-only).** `spa-tracking.js`: nytt `hasInteractionConsent()` (utan cookieless-bypass) + `deferUntilInteractionConsent()`; klick/scroll/konvertering gate:ade och defer:ar tills consent även i cookieless; aggregerad `trackPageView` förblir banner-fri. `requireConsent=false` respekteras fortsatt (bakåtkompatibelt).
...
- [x] **P1-3 `[GDPR]` Publicerad integritetspolicy motsäger cookieless-implementationen.** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16. Kvar (ej blockerande): juridisk granskning av ordalydelsen.** `Privacy.tsx §3B`: nytt "Cookieless JavaScript audience measurement"-avsnitt (in-memory session-id, ingen fingerprint/cross-visit, laglig grund Art. 5.3 + 6.1.f) + explicit att beteendedata (klick/scroll/heatmap/konvertering) kräver consent i alla lägen — stämmer nu med §5.
...
- [~] **P1-4 `[GDPR]` Cloudflare är en oredovisad underleverantör + US-överföring.** `(NY)` — **DOK DEPLOYAD (FTP) 2026-07-16. KVAR (din åtgärd): skicka 30-dagarsvarslet till kunder innan Cloudflare aktiveras + juridisk granskning.** Cloudflare, Inc. (US, EU-US DPF) tillagd som underleverantör i `DPA.md §6`, `GDPR.md`, `Privacy.tsx §5B`; EU-only-påståendena i `DPA.md §7` + `Privacy.tsx §5C` reconcilerade (Cloudflare edge/geo som optional US-överföring). **OBS:** `DPA.md` lovar 30 dagars förhandsvarsel — skicka det innan Cloudflare aktiveras på kundsajter.
...
- [x] **P1-5 `[BIZ]` Install-snippeten regresserade till hårdkodade prod-URL:er → self-host-läcka.** `(NY, regression)` — **✅ DEPLOYAD (FTP) 2026-07-16.** `InstallationGuide.tsx` + `SetupTab.tsx` (Headless + AEM) bygger nu `apiUrl` från `import.meta.env.VITE_SUPABASE_URL` och scriptet från `window.location.origin`.
...
- [x] **P1-6 `[UX][BIZ]` Tracking-mode har dubbel sanning — health-checken kan felaktigt intyga "consent-exempt" medan sajten fortfarande fingerprintar.** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16 (ärlig copy-fix). Robustare single-source kvar som P2-14.** `SetupHealthCheck.tsx` intygar inte längre compliance från enbart DB-toggeln: "Tracking mode (dashboard setting)" + consent-logg-raden säger nu explicit att det speglar dashboarden, att man måste verifiera att det installerade scriptet matchar, och att beteendedata kräver consent. **Full single-source (script/plugin läser `sites.tracking_mode` via ny endpoint) loggad som P2-uppföljning nedan (P2-14).**
...
- [x] **P2-2 `[GDPR]` Cookieless konverterings-capture skickar hashad e-post utan samtycke.** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16 (löst ihop med P1-2).** `recordConversion` har nu `if (!hasInteractionConsent()) return;` + `setupConversionTracking` gate:ad — hashad e-post skickas aldrig utan consent, i något läge.
...
- [x] **P2-7 `[AI]` `llms.txt` säger att CortIQ saknar en feature den har (scroll-heatmaps).** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16.** Scroll-depth-heatmaps flyttade till feature-listan i `llms.txt` + `llms-full.txt`; "does NOT offer" begränsad till spatiala attention/mouse-movement-overlays.
...
- [x] **P2-9 `[UX]` Stale "Version 2.0 — Red theme"-kort på Setup-tabben.** `(NY)` — **✅ DEPLOYAD (FTP) 2026-07-16.** Kortet omskrivet till WP-plugin v5.3.4 (cookieless, Consent Mode v2, geo-gating, i18n, GDPR-safe UX); "red theme"-raden borttagen.
...
- [x] **P2-10 `[BIZ]` Kvarvarande svenska i engelska install-snippets.** `(NY, ersätter delvis gamla P3-12)` — **✅ DEPLOYAD (FTP) 2026-07-16.** Alla svenska strängar i `SetupTab.tsx` (REST-API-kommentarer, AEM-kommentarer, Adobe-hybrid-texten) + `ExternalIntegrationsTab.tsx:52` översatta till engelska.
...
- [ ] **P3-22 `[DB]` Migrations-historik-drift blockerar `supabase db push`.** `(NY, 2026-07-16)` `npx supabase db push` avbryter med "Remote migration versions not found in local migrations directory" och gamla `<timestamp>-<uuid>.sql`-filnamn skippas (kräver `<timestamp>_name.sql`). Följd: DDL måste köras manuellt i dashboardens SQL Editor (så gjordes `20260716120000_event_queue_site_cascade`). **Kör INTE** de föreslagna `migration repair --status reverted …`-kommandona (kan få en framtida push att köra om applicerade migrationer). **Fix:** reconcilera `supabase_migrations.schema_migrations` mot filerna i en egen underhållssession (byt namn på uuid-filer, `migration repair --status applied` för de som verkligen är applicerade). Se projektminnet `cortiq-deployment-state`.

---

## Kvar att göra (uppdaterad 2026-07-16, efter merge + deploy)

**✅ Klart, mergat (PR #9) & deployat 2026-07-16:** P0-1, P1-1, P0-3, P1-2, P1-3, P1-5, P1-6, P1-7, P2-2, P2-7, P2-9, P2-10 — plus bonus delete-site-feature (FTP + `event_queue`-migration). `main` matchar produktion.

**Väntar på DIN åtgärd (icke-blockerande):**
1. **P1-4** — skicka 30-dagarsvarslet om Cloudflare-underleverantören innan det aktiveras på kundsajter (dok är redan live).
2. **P1-3** — juridisk granskning av det nya cookieless-policy-avsnittets ordalydelse (redan live).
3. **P0-2** — kör `/mcp` så verifieras 07-06-deployerna mot remote.

**Ej påbörjat (nästa arbetsblock), föreslagen ordning:**
4. **P2-6** (bot-registret — Comet/Atlas/YouBot m.fl. räknas idag som människor; hänger ihop med deployade P0-1) + **P3-5** (klient-side referrer-map, samma tema).
5. **P2-1** (cloudflare-analytics ägarkoll), **P2-3** (session-id före consent), **P2-4** (cloudflare_traffic-retention), **P2-5** (geo-gating i policy) — P2 säkerhet/GDPR före volym.
6. **P2-8** (MCP-ytan släpar), **P2-11** (ConsentImpact-stat), **P2-14** (full single-source tracking-mode).
7. **P2-13** (kräver ditt Google Ads-beslut), **P2-12** (BYOK-kryptering, rekommenderad defer).
8. **P3-22** (migrations-historik-drift — städa när tid finns; blockerar `supabase db push`).
9. P3 löpande.
