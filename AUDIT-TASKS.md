...
- [x] **P2-3 `[DB]` Dashboard-hooks hämtar obegränsade rader och aggregerar i JS.** FIXAT: `useAttributionGap`, `useLeadQualityPipeline`, `useAIBotTracking` anropar nu SECURITY INVOKER-RPC:er (`get_attribution_gap`, `get_lead_quality_pipeline`, `get_ai_bot_tracking`, migration `dashboard_aggregation_rpcs`, applicerad remote) — RLS scope:ar fortfarande per ägd sajt. Verifierat mot rådata (2902=2902, breakdown-summa=total, training 2851=2851). `useConversionGoalHealth` lämnad medvetet: dess nämnare är redan en `count`-query och resten är låg volym + kräver klientsidans goal-matchning. **Kräver frontend-rebuild + FTP.**
...
- [x] **P2-6 `[SEC][GDPR][DB]` Fingerprint använder icke-krypto 32-bit-hash.** FIXAT: `generateDeviceFingerprint` använder nu `crypto.subtle.digest('SHA-256', …)` med per-sajt-salt (`sites.fingerprint_salt`, migration `fingerprint_salt`, backfillad + default). 128-bitars output, `fp_`-prefix behållet. Server-side only, saltet exponeras aldrig. **Ren brytning:** befintliga besökare re-identifieras en gång (medvetet val pre-scale). **Kräver `supabase functions deploy visitor-identification`.**
...
- [~] **P2-8 `[AI]` Ingen per-user rate limit på `ai-assistant`; upp till 5 Claude-anrop/tur + 100k-radsladdningar.** MESTADELS KLART: månatligt kostnadstak (`ai_usage` + `check_ai_budget`/`record_ai_usage`, enforce:at i `ai-assistant` + `geo-analyze`); **per-user rate limit** (30 turer/timme via `check_rate_limit`); **`cache_control`** på system+tools-prefixet; **`max_tokens` höjt till 8192** + explicit hantering av `stop_reason==='max_tokens'` (returnerar partiellt svar med markör). **Verktygs-aggregering (delvis):** de page_views-baserade verktygen (störst volym/truncerings-risk) — `cortiq_top_pages`, `cortiq_top_exit_pages`, `cortiq_avg_engagement_time` — anropar nu SQL-RPC:er (migration `assistant_pageview_aggregation`, verifierad exakt mot rådata). Kvar i JS medvetet: tracking_sessions-verktygen (få rader/session, ingen närtids-risk) och de JS-specifika (`cortiq_top_sources` URL-hostname-parse, `cortiq_top_entry_pages` cappad tvåstegs-join). **Bugg upptäckt + FIXAD:** `cortiq_web_vitals` selectade `lcp`/`cls` från `page_views` som saknar dem → verktyget var trasigt. Nu pekar det på rätt tabell (`web_vitals`, kolumner `page_url`/`lcp`/`measured_at`) via SQL-RPC `assistant_web_vitals` (migration `assistant_web_vitals`). Kräver `supabase functions deploy ai-assistant`.
