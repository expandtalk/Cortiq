...
- [ ] **P2-2 `[DB]` RLS re-evaluerar `auth.uid()` per rad på stora tabeller.** KVAR: `unified_visitors` (SELECT-policy, `20260210000000_unified_visitors.sql:246`) och `conversion_events` (`20250719041946:40`) använder fortfarande bar `auth.uid()`. Wrappa som `(select auth.uid())`. (Använder redan EXISTS.)
...
- [ ] **P2-9 `[AI]` Bot-taxonomin driftar mellan ingest och dashboard.** KVAR: `ai-bot-tracker/index.ts:21` (`AI_BOT_REGISTRY`) och `BotTrafficClassification.tsx` (`TRAINING_PATTERNS`/`AGENTIC_PATTERNS`) är fortfarande två oberoende listor (dashboard-kommentaren säger "MUST stay in sync"). Extrahera en delad registry-modul. Uppdatera även för 2026: Grok, DuckAssistBot, Meta-ExternalFetcher, YouBot m.fl.
...
- [ ] **P2-16 `[UX]` Aktiv tabb är komponent-lokal state, inte i URL.** KVAR: `DashboardTabs.tsx:146` `useState('overview')`, ingen `useSearchParams`. Synka `activeTab` till URL-param via react-router.
