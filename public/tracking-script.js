...
(function() {
  ...
  class HeatmapAnalyticsTracker {
    ...
    isAdminElement(element) {
      ...
      // Kolla URL först - skippa allt på admin-sidor
      ...
      // Kolla element text för admin-relaterade termer
      const text = element.textContent?.toLowerCase() || '';
      const adminKeywords = [
        'redigera', 'edit', 'admin', 'adminpanel', 'admin panel',
        'dashboard', 'backend', 'wp-admin', 'login', 'logout', 
        'logga ut', 'redigera inlägg', 'edit post', 'add new',
        'lägg till', 'delete', 'ta bort', 'publish', 'publicera',
        'draft', 'utkast', 'preview', 'förhandsgranska',
        'settings', 'inställningar', 'users', 'användare',
        'comments', 'kommentarer', 'media', 'plugins', 'themes',
        'tools', 'verktyg', 'customize', 'anpassa'
      ];
      
      if (adminKeywords.some(keyword => text.includes(keyword))) {
        return true;
      }
      
      // Kolla element attribut
      const id = element.id?.toLowerCase() || '';
      const className = element.className?.toLowerCase() || '';
      const href = element.href?.toLowerCase() || '';
      
      if (id.includes('admin') || id.includes('wp-') ||
          className.includes('admin') || className.includes('wp-') ||
          href.includes('/wp-admin/') || href.includes('/admin/')) {
        return true;
      }
      
      // Kolla parent elements
      ...
      while (parent && depth < 3) {
        ...
        if (parentClass.includes('admin') || parentClass.includes('wp-') ||
            parentId.includes('admin') || parentId.includes('wp-')) {
          return true;
        }
    ...
    trackPageView() {
      ...
      if (window.location.href.includes('/wp-admin/') || 
          window.location.href.includes('/admin/') ||
          window.location.href.includes('/dashboard/')) {
        console.log('Heatmap Analytics: Skippar admin-sida', window.location.href);
        return;
      }
    ...
    trackClick(event) {
      ...
      // Skip admin-related elements
      if (this.isAdminElement(element)) {
        console.log('Heatmap Analytics: Skippar admin-element', element);
        return;
      }
    ...
    flushEvents() {
      ...
      const payload = {
        type: 'batch',
        siteId: this.config.siteId || this.config.trackingId,  // Använd siteId från config
